...
func (o OopsErrorBuilder) Wrap(err error) error {
	...
	o2 := o.copy()
	o2.err = err
	if o2.trace == "" && AutoTraceID {
		o2.trace = ulid.Make().String()
		o2.traceAutoGenerated = true
	}
	if o2.span == "" {
...
func (o OopsErrorBuilder) Wrapf(err error, format string, args ...any) error {
	...
	o2 := o.copy()
	o2.err = err
	// Use fmt.Errorf only when %w is present to handle the wrapping verb.
	// Note: when %w is used, the wrapped error from fmt.Errorf is discarded —
	// only the formatted string is kept. The wrapped error is preserved via o2.err.
	if strings.Contains(format, "%w") {
		o2.msg = fmt.Errorf(format, args...).Error()
	} else {
		o2.msg = fmt.Sprintf(format, args...)
	}
	if o2.trace == "" && AutoTraceID {
		o2.trace = ulid.Make().String()
		o2.traceAutoGenerated = true
...
func (o OopsErrorBuilder) Errorf(format string, args ...any) error {
	o2 := o.copy()
	// Use fmt.Errorf only when %w is present to support error wrapping.
	// When no %w verb is used, errors.New(fmt.Sprintf(...)) avoids the
	// heavier allocation of a fmt.wrapError struct.
	if strings.Contains(format, "%w") {
		o2.err = fmt.Errorf(format, args...)
	} else {
		o2.err = errors.New(fmt.Sprintf(format, args...))
	}
	if o2.trace == "" && AutoTraceID {
		o2.trace = ulid.Make().String()
		o2.traceAutoGenerated = true
