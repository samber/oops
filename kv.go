...
// snapshot traverses the error chain once and returns a fully resolved,
...
func snapshot(o OopsError) OopsError { //nolint:gocyclo
	...
	recursive(o, func(e OopsError) bool {
		...
		s.tags = append(s.tags, e.tags...)
		if e.trace != "" {
			if e.traceAutoGenerated {
				autoTrace = e.trace
			} else {
				explicitTrace = e.trace
			}
		}
		if e.hint != "" {
			s.hint = e.hint
		...
		if len(e.context) > 0 {
			contextMaps = append(contextMaps, e.context)
		}
		if e.userID != "" {
			s.userID = e.userID
		}
		if len(e.userData) > 0 {
			userDataMaps = append(userDataMaps, e.userData)
		}
		if e.tenantID != "" {
			s.tenantID = e.tenantID
		}
		if len(e.tenantData) > 0 {
			tenantDataMaps = append(tenantDataMaps, e.tenantData)
		}
		if e.req != nil {
			s.req = e.req
		}
